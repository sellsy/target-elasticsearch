...
class ElasticSink(BatchSink):
    ...
    def __init__(
        ...
        self._compiled_metadata_fields = {
            k: jsonpath_ng.parse(v)
            for k, v in (self.metadata_fields or {}).items()
        }
        self._compiled_index_schema_fields = {
            k: jsonpath_ng.parse(v)
    ...
    def build_request_body_and_distinct_indices(
        ...
        for record in records:
            if self.index_schema_fields:
                index = self._template_index(self._build_fields(self.index_schema_fields, record, self._compiled_index_schema_fields))
            ...
            if self.metadata_fields is not None:
                ...
                metadata_fields = self._build_fields(self.metadata_fields, record, self._compiled_metadata_fields)
